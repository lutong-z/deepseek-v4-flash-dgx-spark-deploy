...
ALLOWED_ENV_KEYS = {
    ...
    "OBS_WORKER_SSH_USER",
    "OBS_SSH_PORT",
    "OBS_SSH_KNOWN_HOSTS_FILE",
    "OBS_SSH_IDENTITY_FILE",
    "OBS_REMOTE_ROOT",
    "OBS_IMAGE_LOCK_FILE",
    "OBS_PROFILE_FILE",
    "OBS_GRAFANA_ADMIN_USER",
    "OBS_GRAFANA_ADMIN_PASSWORD",
    "OBS_BIND_ADDR",
    "OBS_VLLM_PORT",
    "OBS_VLLM_METRICS_PATH",
    "OBS_VLLM_SCHEME",
    "OBS_HEAD_NODE_NAME",
    "OBS_WORKER_NODE_NAME",
    "OBS_VLLM_CONTAINER_HEAD",
    "OBS_VLLM_CONTAINER_WORKER",
    "OBS_VLLM_LOG_PATH_HEAD",
    "OBS_VLLM_LOG_PATH_WORKER",
    "OBS_FABRIC_HCA",
    "OBS_FABRIC_NDEV",
    "OBS_FABRIC_GID_INDEX",
    "OBS_DOCKER_PROXY",
    "OBS_SKIP_VLLM_CHECK",
...
def _config(
    ...
    *,
    ...
    ports = network["ports"]
    head_host = _validate_host(
        _first(env, "OBS_HEAD_HOST", "HEAD_HOST", default=str(network["head_addr"])),
        "OBS_HEAD_HOST",
    )
    worker_host = _validate_host(
        _first(
            env, "OBS_WORKER_HOST", "WORKER_HOST", default=str(network["worker_addr"])
        ),
        "OBS_WORKER_HOST",
    )
    head_ssh = _validate_host(
        _first(env, "OBS_HEAD_SSH_HOST", "HEAD_SSH_HOST", default=head_host),
        "OBS_HEAD_SSH_HOST",
    )
    worker_ssh = _validate_host(
        _first(env, "OBS_WORKER_SSH_HOST", "WORKER_SSH_HOST", default=worker_host),
        "OBS_WORKER_SSH_HOST",
    )
    ...
    ssh_port = _int_value(
        _first(env, "OBS_SSH_PORT", "SSH_PORT", default="22"), "OBS_SSH_PORT"
    )
    remote_root_value = _first(
        env, "OBS_REMOTE_ROOT", default="/tmp/dgx-spark-observability-render"
    )
    remote_root = _validate_abs_path(
        remote_root_value, "OBS_REMOTE_ROOT", external=require_remote
    )
    ...
    vllm_port = _int_value(
        _first(env, "OBS_VLLM_PORT", "API_PORT", default="8101"), "OBS_VLLM_PORT"
    )
    if vllm_port != 8101:
        _fail("OBS_VLLM_PORT must remain 8101; production vLLM is not changed")
    metrics_path = _first(
        env, "OBS_VLLM_METRICS_PATH", default=str(profile["service"]["metrics_path"])
    )
    if not metrics_path.startswith("/") or any(
        char in metrics_path for char in " \t\r\n'\""
    ):
    ...
    containers = {
        key: _safe_component_name(str(value["container"]))
        for key, value in profile["components"].items()
    }
    # Default to the reviewed dspark5 rollout names; any valid container name
    # is accepted so later (e.g. lmcache) rollouts can reuse the same stack.
    ...
    container_names = {
        "head": _first(
            env, "OBS_VLLM_CONTAINER_HEAD", default=expected_vllm_containers["head"]
        ),
        "worker": _first(
            env, "OBS_VLLM_CONTAINER_WORKER", default=expected_vllm_containers["worker"]
        ),
    }
    log_paths = {
    ...
    for role, name in container_names.items():
        container_names[role] = _safe_component_name(name)
    for role, path in log_paths.items():
        if path and (
            not path.startswith("/") or any(char in path for char in "*?[\\r\\n'\"")
        ):
            _fail(
                f"OBS_VLLM_LOG_PATH_{role.upper()} must be an exact absolute file path"
            )
    fabric_hca = _first(env, "OBS_FABRIC_HCA", default=str(network.get("hca", "")))
    fabric_ndev = _first(
        env, "OBS_FABRIC_NDEV", default=str(network.get("interface", ""))
    )
    fabric_gid_index = _first(env, "OBS_FABRIC_GID_INDEX", default="")
    for key, value in (
        ("OBS_FABRIC_HCA", fabric_hca),
        ("OBS_FABRIC_NDEV", fabric_ndev),
        ("OBS_FABRIC_GID_INDEX", fabric_gid_index),
    ):
    ...
    return {
        ...
        "ssh_port": ssh_port,
        "ssh_known_hosts": _first(
            env, "OBS_SSH_KNOWN_HOSTS_FILE", "SSH_KNOWN_HOSTS_FILE"
        ),
        ...
        "volumes": dict(profile["volumes"]),
        "head_node_name": _first(env, "OBS_HEAD_NODE_NAME", default="dgx-head"),
        "worker_node_name": _first(env, "OBS_WORKER_NODE_NAME", default="dgx-worker"),
        "vllm_container_names": container_names,
        "vllm_log_paths": log_paths,
        "fabric_hca": fabric_hca,
        "fabric_ndev": fabric_ndev,
        "fabric_gid_index": fabric_gid_index,
        "profile_id": str(profile["profile_id"]),
        "retention": dict(profile.get("retention", {})),
        "intervals": dict(profile.get("intervals", {})),
        "images": dict(lock or {}),
...
def render_promtail(config: Mapping[str, Any], role: str) -> str:
    if role == "head":
        ...
        target, node = (
            f"http://{config['head_host']}:{config['ports']['loki']}/loki/api/v1/push",
            config["worker_node_name"],
        )
...
def render_grafana_dashboard() -> str:
    dashboard = {
        "id": None,
        "uid": "dgx-spark-overview",
        "title": "DGX Spark overview",
        "schemaVersion": 39,
        "version": 2,
        "refresh": "15s",
        ...
        "panels": [
            ...
            },
            {
                "id": 18,
                "type": "stat",
                "title": "Token totals (selected range)",
                "description": (
                    "Counter increases over the active Grafana time range. "
                    "Cached input includes local prefix-cache hits and external "
                    "KV transfers."
                ),
                "gridPos": {"h": 8, "w": 6, "x": 18, "y": 0},
                "fieldConfig": {
                    "defaults": {"unit": "short", "decimals": 1, "min": 0},
                    "overrides": [],
                },
                "options": {
                    "colorMode": "none",
                ...
                "targets": [
                    ...
                    {
                        "expr": (
                            "sum(increase(vllm:prompt_tokens_cached_total[$__range])) "
                            "or vector(0)"
                        ),
                        "instant": True,
                        "refId": "C",
            ...
            },
            {
                "id": 3,
                "type": "timeseries",
                "title": "vLLM request throughput",
                "gridPos": {"h": 8, "w": 12, "x": 0, "y": 8},
                "targets": [
                    {"expr": "rate(vllm:request_success_total[5m])", "refId": "A"}
                ],
            },
            {
                "id": 4,
            ...
            },
            {
                ...
                "targets": [
                    {"expr": "rate(vllm:num_preemptions_total[5m])", "refId": "A"}
                ],
            },
            {
                "id": 9,
                "type": "timeseries",
                "title": "F1 GID validity",
                "gridPos": {"h": 8, "w": 12, "x": 0, "y": 32},
                "targets": [
                    {"expr": "dgx_fabric_gid_valid", "refId": "A"},
                    {"expr": "dgx_fabric_rocev2_gid", "refId": "B"},
                ],
            },
            {
                "id": 10,
                "type": "timeseries",
                "title": "F1 link and MTU",
            ...
            },
            {
                "id": 11,
                "type": "logs",
                "title": "NCCL/RoCE errors",
                "gridPos": {"h": 8, "w": 24, "x": 0, "y": 40},
                "targets": [
                    {
                        "expr": '{job="vllm"} |~ "(?i)NCCL|RoCE|RDMA|timeout|preempt"',
                        "refId": "A",
                        "datasource": {"type": "loki", "uid": "dgx-loki"},
                    }
                ],
            },
            {
                "id": 12,
                "type": "timeseries",
                "title": "External KV cache hit rate (LMCache)",
                ...
                "targets": [
                    {
                        "expr": "sum(rate(vllm:external_prefix_cache_hits_total[5m])) / sum(rate(vllm:external_prefix_cache_queries_total[5m]))",
                        "refId": "A",
                        "legendFormat": "external hit rate",
                    }
                ],
            },
            {
                "id": 13,
                "type": "timeseries",
                "title": "Local prefix cache hit rate",
            ...
            },
            {
                "id": 15,
                "type": "timeseries",
                "title": "Prompt tokens by source (cache-hit input)",
                "gridPos": {"h": 8, "w": 12, "x": 12, "y": 56},
                "fieldConfig": {"defaults": {"unit": "tps"}, "overrides": []},
                "targets": [
                    {
                        "expr": "sum by (source) (rate(vllm:prompt_tokens_by_source_total[5m]))",
                        "refId": "A",
                        "legendFormat": "{{source}}",
                    },
                    {
                        "expr": "sum(rate(vllm:external_prefix_cache_hits_total[5m]))",
                        "refId": "B",
                        "legendFormat": "external hit input tokens/s",
                    },
                ],
            },
            {
                "id": 16,
                "type": "timeseries",
                "title": "Prefix cache expiry",
                "gridPos": {"h": 8, "w": 12, "x": 0, "y": 64},
                "targets": [
                    {
                        "expr": "sum(rate(vllm:prefix_cache_expired_blocks_total[5m]))",
                        "refId": "A",
                        "legendFormat": "GPU blocks/s",
                    },
                    {
                        "expr": "sum(rate(dgx_lmcache_l2_expired_entries_total[5m]))",
                        "refId": "B",
                        "legendFormat": "L2 objects/s",
                    },
                ],
            },
            {
                "id": 17,
                "type": "timeseries",
                "title": "LMCache L2 footprint",
                "gridPos": {"h": 8, "w": 12, "x": 12, "y": 64},
                "fieldConfig": {"defaults": {"unit": "bytes"}, "overrides": []},
                "targets": [
                    {
                        "expr": "dgx_lmcache_l2_bytes",
                        "refId": "A",
                        "legendFormat": "{{instance}} bytes",
                    },
                    {
                        "expr": "dgx_lmcache_l2_entries",
                        "refId": "B",
...
def render_files(config: Mapping[str, Any]) -> dict[str, bytes]:
    ...
    files.update(
        {key: value.encode() for key, value in render_grafana_provisioning().items()}
    )
...
def _run_local(
    args: Sequence[str], *, input_text: str | None = None, dry_run: bool = False
) -> str:
    if dry_run:
        print("DRY-RUN", " ".join(_quote(str(item)) for item in args))
        if input_text:
            safe = re.sub(
                r"GF_SECURITY_ADMIN_PASSWORD=.*",
                "GF_SECURITY_ADMIN_PASSWORD=[secret omitted]",
                input_text,
            )
            print(safe.rstrip())
        return ""
...
def _remote_script(
    config: Mapping[str, Any], role: str, action: str, *, purge: bool = False
) -> str:
    ...
    lines = [
        ...
        "set -euo pipefail",
        f"ROOT={_quote(root)}",
        f"NETWORK={_quote(network_name)}",
        "LABEL_KEY=com.dgx-spark.observability",
    ...
    if action == "down":
        for component in components:
            ...
            lines.append(
                f"if owned_container {_quote(name)}; then docker rm -f {_quote(name)} >/dev/null; fi"
            )
        lines += [
        ...
        if purge:
            purge_keys = (
                ...
                if role == "head"
                else ("promtail_worker",)
            )
            for key in purge_keys:
                lines.append(
                    f"if docker volume inspect {_quote(volumes[key])} >/dev/null 2>&1; then owned_volume {_quote(volumes[key])}; docker volume rm {_quote(volumes[key])} >/dev/null; fi"
                )
        return "\n".join(lines) + "\n"
    if action == "status":
        lines.append(
            "docker ps -a --filter label=\"$LABEL_KEY=$LABEL\" --format '{{.Names}}\\t{{.Status}}\\t{{.Image}}' || true"
        )
        return "\n".join(lines) + "\n"
    if action == "verify":
        for component in components:
    ...
    for component in components:
        name = containers[component]
        lines.append(
            f"if docker inspect {_quote(name)} >/dev/null 2>&1; then owned_container {_quote(name)}; docker rm -f {_quote(name)} >/dev/null; fi"
        )
    volume_keys = (
        ("prometheus", "grafana", "alertmanager", "loki", "promtail_head")
        if role == "head"
    ...
    def run(
        ...
        lines.append(
            f'if ! docker image inspect "${image_var}" >/dev/null 2>&1 && docker image inspect {_quote(cache_tag)} >/dev/null 2>&1; then {image_var}={_quote(cache_tag)}; fi'
        )
        docker_run = "docker run -d --restart unless-stopped"
        if docker_proxy:
            docker_run = (
                'HTTPS_PROXY="$DOCKER_PROXY" HTTP_PROXY="$DOCKER_PROXY" ' + docker_run
            )
        parts = [
            docker_run,
            f"--name {_quote(name)}",
            '--label "$LABEL_KEY=$LABEL"',
            f"--label {_quote('com.dgx-spark.observability.component=' + name)}",
            '--network "$NETWORK"',
        ]
        network_alias = {
            "loki": "loki",
    ...
    run(
        containers[fabric_key],
        "fabric_exporter",
        "python3 /opt/fabric_exporter.py",
...
def _remote_prepare(
    config: Mapping[str, Any], role: str, files: Mapping[str, bytes], *, dry_run: bool
) -> None:
    root = config["remote_root"]
    ...
    if not dry_run:
        if dirs:
            mkdir = (
                "set -eu\n"
                + "\n".join(
                    f"mkdir -p {_quote(root + '/config/' + directory)}"
    ...
    for relative, content in role_files.items():
        ...
        with tempfile.NamedTemporaryFile(
            prefix="dgx-observe-", suffix=".tmp", delete=False
        ) as temporary:
        ...
        try:
            ...
            _run_local(
                _scp_args(config, role)[:-1] + [str(temporary_path), destination]
            )
        finally:
    ...
    if not dry_run:
        ...
        permissions.extend(
            f"chmod 755 {_quote(root + '/config/' + directory)}" for directory in dirs
        )
        permissions.extend(
            f"chmod 644 {_quote(root + '/config/' + relative)}"
            for relative in role_files
        )
    ...
    if role == "head":
        if dry_run:
            ...
            with tempfile.NamedTemporaryFile(
                prefix="dgx-grafana-", suffix=".env", delete=False
            ) as temporary:
...
def _remote_proxy_prepare(
    config: Mapping[str, Any], role: str, *, dry_run: bool
) -> None:
    ...
    _run_local(
        _ssh_args(config, role) + ["bash", "-s"], input_text=info_check, dry_run=dry_run
    )
...
def _stage_worker_images(config: Mapping[str, Any], *, dry_run: bool) -> None:
    ...
    pull_save.append(
        f'docker save --output "$ARCHIVE" {" ".join(_quote("dgx-observe-cache:" + key) for key, _ in refs)}'
    )
    _run_local(
    ...
    with tempfile.NamedTemporaryFile(
        prefix="dgx-observability-images-", suffix=".tar", delete=False
    ) as temporary:
    ...
    *,
    ...
    if action == "up":
        ...
        for role in ("head", "worker"):
            ...
            _run_local(
                _ssh_args(config, role) + ["bash", "-s"],
                input_text=_remote_script(config, role, "up"),
                dry_run=dry_run,
            )
        return
    for role in ("head", "worker"):
        output = _run_local(
            _ssh_args(config, role) + ["bash", "-s"],
            input_text=_remote_script(config, role, action, purge=purge),
            dry_run=dry_run,
        )
        if output:
            print(f"[{role}]\n{output}", end="")
...
def _load_inputs(
    args: argparse.Namespace, *, need_lock: bool
) -> tuple[dict[str, str], dict[str, Any], dict[str, str] | None]:
    env = parse_env(Path(args.env_file)) if args.env_file else {}
    profile = load_profile(
        Path(args.profile or env.get("OBS_PROFILE_FILE") or DEFAULT_PROFILE)
    )
...
def _check_vllm(config: Mapping[str, Any], *, dry_run: bool = False) -> None:
    ...
    for path in paths:
        lines.append(
            f"curl --fail --silent --show-error --max-time 15 {_quote(base + path)} >/dev/null"
        )
    _run_local(
        _ssh_args(config, "head") + ["bash", "-s"],
        input_text="\n".join(lines) + "\n",
...
def cmd_check_vllm(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        print(
            "production vLLM /health, /v1/models, /metrics, and one-token smoke verified from DGX head"
        )
    else:
        print(
            "production vLLM /health, /v1/models, and /metrics verified read-only from DGX head"
        )
    return 0
...
def cmd_deploy(args: argparse.Namespace) -> int:
    ...
    # OBS_SKIP_VLLM_CHECK=1 lets operators stand up the monitoring stack
    # before the model service exists (or while it is briefly down); the
    # stack then starts scraping vLLM as soon as the service appears. The
    # Prometheus rule `up{job="vllm"} == 0` will honestly report the gap.
    if env.get("OBS_SKIP_VLLM_CHECK") != "1":
        _check_vllm(config, dry_run=args.dry_run)
    ...
    if not args.dry_run:
        if env.get("OBS_SKIP_VLLM_CHECK") != "1":
            _check_vllm(config)
    print(
        "dry-run complete; no DGX or Docker mutation performed"
        if args.dry_run
        else "observability stack deployed on DGX hosts; production vLLM health remained intact"
    )
    return 0
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    if not args.dry_run:
        ...
        for name, port, path in checks:
            ...
            lines.append(
                f"if ! curl --fail --silent --show-error --max-time 10 {_quote(url)} >/dev/null; then echo {_quote('observability endpoint failed: ' + name)} >&2; exit 1; fi"
            )
        for host in (config["head_host"], config["worker_host"]):
            node_url = _quote(
                ...
                + host
                + ":"
                + str(config["ports"]["node_exporter"])
                + "/metrics"
            )
            fabric_url = _quote(
                "http://"
                + host
                + ":"
                + str(config["ports"]["fabric_exporter"])
                + "/metrics"
            )
            lines.append(
                f"node_metrics=$(curl --fail --silent --show-error --max-time 10 {node_url})"
            )
            lines.append(
                f'if [[ "$node_metrics" != *"node_exporter_build_info"* ]]; then echo {_quote("node exporter failed: " + host)} >&2; exit 1; fi'
            )
            lines.append(
                f"if ! fabric_metrics=$(curl --fail --silent --show-error --max-time 10 {fabric_url}); then echo {_quote('fabric exporter failed: ' + host)} >&2; exit 1; fi"
            )
            for metric in (
                "dgx_fabric_gid_valid",
                "dgx_fabric_gid_ipv4_mapped",
                "dgx_fabric_gid_type_info",
                "dgx_fabric_ndev_match",
                "dgx_fabric_rocev2_gid",
                "dgx_fabric_mtu",
                "dgx_fabric_rdma_link_up",
                "dgx_fabric_link_up",
            ):
                lines.append(
                    f'if [[ "$fabric_metrics" != *"{metric}"* ]]; then echo {_quote(metric + " missing: " + host)} >&2; exit 1; fi'
                )
        vllm_url = (
            config["scheme"]
            + "://"
        ...
        lines.append(
            f"if ! curl --fail --silent --show-error --max-time 10 {_quote(vllm_url)} >/dev/null; then echo 'vLLM metrics endpoint failed' >&2; exit 1; fi"
        )
        _run_local(
            _ssh_args(config, "head") + ["bash", "-s"],
            input_text="\n".join(lines) + "\n",
        ...
        print(
            "observability endpoints, exporters, and vLLM metrics verified from DGX head"
        )
    return 0
...
def cmd_down(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        print(
            "observability containers removed; persistent volumes "
            + ("purged" if args.purge_data else "retained")
        )
    return 0
...
def cmd_resolve_images(args: argparse.Namespace) -> int:
    ...
    if not isinstance(image_entries, dict) or set(image_entries) != set(
        DEFAULT_IMAGE_TAGS
    ):
        _fail("DGX image resolver returned incomplete image set")
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(
        json.dumps(_lock_document(image_entries), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    output.chmod(0o600)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def common(command: argparse.ArgumentParser, *, env_required: bool = False) -> None:
        command.add_argument(
            ...
            help="operator-owned KEY=VALUE file outside checkout",
        )
        command.add_argument(
            "--profile",
            help="observability profile JSON (default: committed DGX profile)",
        )

    render = sub.add_parser(
        "render", help="render configs locally without SSH or Docker"
    )
    common(render)
    ...
    render.add_argument(
        ...
        action="store_true",
        help="render only; never valid for deploy",
    )
    render.add_argument("--output", required=True)
    render.set_defaults(func=cmd_render)

    deploy = sub.add_parser(
        "deploy", help="deploy labelled observability containers on both DGX hosts"
    )
    ...
    deploy.add_argument(
        "--confirm",
        required=True,
        help="type DGX-OBSERVABILITY to authorize remote mutation",
    )
    deploy.add_argument("--dry-run", action="store_true")
    deploy.set_defaults(func=cmd_deploy)
    check = sub.add_parser(
        "check-vllm",
        help="read-only production vLLM health, models, and metrics check from DGX head",
    )
    ...
    check.add_argument(
        "--smoke-one-token",
        action="store_true",
        help="run one deterministic completion token after endpoint checks",
    )
    check.set_defaults(func=cmd_check_vllm)

    resolve = sub.add_parser(
        "resolve-images", help="resolve public image tags to ARM64 RepoDigests on DGX"
    )
    ...
    for name, func, help_text in (
        ("status", cmd_status, "show labelled observability containers"),
        ("verify", cmd_verify, "verify containers and read-only health endpoints"),
        ("down", cmd_down, "remove labelled observability containers"),
    ):
